...
const SUPPORTED_REMINDER_HOURS = [1, 2, 4]
// ── Handler ────────────────────────────────────────────────────────────────────
export const sendMatchReminders = onSchedule({
  ...
  await Promise.all(SUPPORTED_REMINDER_HOURS.map(hours => _processWindow(now, hours)))
})
// ── Internos ──────────────────────────────────────────────────────────────────
/**
 ...
 */
async function _processWindow(now, hoursAhead) {
  ...
  for (const matchDoc of matchesSnap.docs) {
    ...
    console.log(`[matchReminders] Partido ${matchDoc.id}: enviados=${result.sent}, fallidos=${result.failed}`)
  }
}
