 ...
 */
export async function sendToTokens(tokens, payload, userIds) {
  if (!tokens.length)
 ...
 */
export async function sendNotification(type, data, tokens, userIds) {
...
// ── Internos ──────────────────────────────────────────────────────────────────
async function _cleanInvalidTokens(items) {
  const db = admin.firestore()
  const { FieldValue } = admin.firestore
  const batches = []
  for (const { userId, token } of items) {
    batches.push(db.collection('users').doc(userId).update({
